...
def dfs(rr, rc, br, bc, count = 0):
    global answer
    if (rr, rc) == (gr, gc) or count > 10:
        answer = min(answer, count)
        return

    def isMovable(r, c, dr, dc, er, ec):
        print(f"isMovable 함수: r: {r}, c: {c}, dr: {dr}, dc: {dc}, er: {er}, ec: {ec}")
        if 0 <= r + dr < n and 0 <= c + dc < m and (r+dr, c+dc) != (er, ec) and maze[r + dr][c + dc] != "#":
            return 1
        return 0
    ...
    for dr, dc in d:
        if isMovable(rr, rc, dr, dc, br, bc) or isMovable(br, bc, dr, dc, rr, rc): #빨간 구슬이 움직일 수 있는 경우
            ...
            if (trr, trc) == (gr, gc):
                trr, trc = -1, -1
            tbr, tbc = move(br, bc, dr, dc, trr, trc)
            trr, trc = move(rr, rc, dr, dc, tbr, tbc)
            if (tbr, tbc) != (gr, gc):
                dfs(trr, trc, tbr, tbc, count + 1)
